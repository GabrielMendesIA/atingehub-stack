...
**Windows (winget, run in git bash):**
```bash
winget install --id Python.Python.3.12 -e --source winget --accept-package-agreements --accept-source-agreements --silent
```
The binary lands at `/c/Users/<user>/AppData/Local/Programs/Python/Python312/python.exe`. The current bash session won't have it on PATH — use the full path for the venv creation below.
...
**Windows note:** the venv puts entrypoints in `Scripts/` instead of `bin/`, so skip `source activate` and use the venv python directly:
```bash
PYEXE="/c/Users/$USER/AppData/Local/Programs/Python/Python312/python.exe"
"$PYEXE" -m venv "$HOME/.notebooklm-venv"
VPY="$HOME/.notebooklm-venv/Scripts/python.exe"
"$VPY" -m pip install --upgrade pip
"$VPY" -m pip install "notebooklm-py[browser]"
"$VPY" -m playwright install chromium
```

Then expose `notebooklm` on PATH.
...
**Windows (wrapper script, not symlink — symlinks need admin):**
```bash
mkdir -p ~/bin
cat > ~/bin/notebooklm << 'SHEOF'
#!/usr/bin/env bash
export PYTHONIOENCODING=utf-8
export PYTHONUTF8=1
exec "$HOME/.notebooklm-venv/Scripts/notebooklm.exe" "$@"
SHEOF
chmod +x ~/bin/notebooklm
```
The `PYTHONIOENCODING`/`PYTHONUTF8` env vars are **critical** on Windows — without them the `rich` library crashes with `UnicodeEncodeError` trying to print `✓` through the cp1252 console.
...
Write and run this login script. **Use `$HOME/.notebooklm/` paths (not `/tmp/`) — cross-platform and the file is reusable for re-login.**

```bash
mkdir -p "$HOME/.notebooklm"
cat > "$HOME/.notebooklm/login_tmp.py" << 'PYEOF'
import json, time
...
# Run the login script in the background (use venv python directly — works on all platforms)
# macOS/Linux: ~/.notebooklm-venv/bin/python
# Windows:     ~/.notebooklm-venv/Scripts/python.exe
VPY="$HOME/.notebooklm-venv/Scripts/python.exe"
[ -f "$VPY" ] || VPY="$HOME/.notebooklm-venv/bin/python"
"$VPY" "$HOME/.notebooklm/login_tmp.py" > "$HOME/.notebooklm/login_output.txt" 2>&1 &
echo "Login started (PID=$!). Browser should open in a few seconds..."
```

Wait ~8 seconds for the browser to open (read `$HOME/.notebooklm/login_output.txt` to confirm it reached "Browser is open. Waiting for save signal..."), then ask the user if they can see the browser and are signed in.

Once the user confirms they are on the NotebookLM homepage, save the session:

```bash
touch "$HOME/.notebooklm/save_signal"
cat "$HOME/.notebooklm/login_output.txt"
```
...
If auth passes (SID cookie present), confirm to the user that NotebookLM is set up and ready. Clean up only the signal/output files — **keep `login_tmp.py` so the user can re-login without rewriting the script**:

```bash
rm -f "$HOME/.notebooklm/save_signal" "$HOME/.notebooklm/login_output.txt"
```

For subsequent re-logins, rerun:
```bash
VPY="$HOME/.notebooklm-venv/Scripts/python.exe"
[ -f "$VPY" ] || VPY="$HOME/.notebooklm-venv/bin/python"
"$VPY" "$HOME/.notebooklm/login_tmp.py" > "$HOME/.notebooklm/login_output.txt" 2>&1 &
